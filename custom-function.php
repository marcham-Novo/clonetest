<?php
    ...
    function custom_scripts() {
        ...
        wp_enqueue_script( 'smoothing-scroll', get_stylesheet_directory_uri() . '/js/smoothing-scroll.js', array('jquery'), '1.0' ); 
    } 
    
    // stickup     
    add_filter( 'cherry_stickmenu_selector', 'cherry_change_selector' );
    	function cherry_change_selector($selector) {
    		$selector = '.extra_head2';
    		return $selector;
   	}
				...
				$output .= '<li class="'. $spans .' list-item-'.$count.'">';
					...
					if ($meta == 'yes') {
						...
						$output .= '<div class="post_meta">';
							...
							$output .= '<time datetime="'.get_the_time('Y-m-d\TH:i:s', $post_id).'">' .get_the_date("j"). '</time>';
							$output .= '<em><time datetime="'.get_the_time('Y-m-d\TH:i:s', $post_id).'">' .get_the_date("M"). '</time></em>';
							$output .= '</span>';
				...
				if ( have_posts() ) {
					...
					while( have_posts() ) {
						...
						$output .= $display_link ? '<h3><a href="'.get_permalink().'" >'.get_the_title().'</a></h3>': '' ;
							$output .= get_field('geolocalisation');
                        $output .= my_string_limit_words($excerpt,8);
						$output .= '</li>';
					}
					$output .= '</ul></div>';
					echo $output;
